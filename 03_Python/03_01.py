...
def find_nearest_crossing(path_1, path_2, expected_val=0):
    trail_1 = create_trail(path_1)
    trail_2 = create_trail(path_2)

    crossings = []
    #print("Trail 1: " + str(len(trail_1)))
    #print("Trail 2: " + str(len(trail_2)))

    for idx, mark in enumerate(trail_1):
        if (idx%100) == 0:
            #print(idx) 
            pass  
        if mark in trail_2:
            crossings.append(mark)
            continue

    minimum = 9999999
    for crossing in crossings[1:]:
        sum = abs(crossing[0]) + abs(crossing[1])
        if sum < minimum:
            minimum = sum
