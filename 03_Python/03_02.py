...
test_11 = ["R75", "D30", "R83", "U83", "L12", "D49", "R71", "U7","L72"]
test_12 = ["U62", "R66", "U55", "R34", "D71","R55", "D58", "R83"]
expected_test_val_1 = 610

test_21 = ["R98", "U47", "R26", "D63", "R33", "U87", "L62", "D20", "R33", "U53", "R51"]
test_22 = ["U98", "R91", "D20", "R16", "D67", "R40", "U7", "R15", "U6", "R7"]
expected_test_val_2 = 410

def create_trail(path):
    trail = [[0, 0, 0]]
    for idx, instruction in enumerate(path):
...
def remove_idx(path):
    new_path = []
    for mark in path:
        new_path.append(mark[:-1])
    return new_path

def find_nearest_crossing(path_1, path_2, expected_val=0):
    trail_1_idx = create_trail(path_1)
    trail_2_idx = create_trail(path_2)

    trail_1 = remove_idx(trail_1_idx)
    trail_2 = remove_idx(trail_2_idx)

    crossings = []

    for idx1, mark1 in enumerate(trail_1):
        if (idx1%100) == 0:
            print(idx1) 
        for idx2, mark2 in enumerate(trail_2):
            if mark1 == mark2:
                mark1.append(idx1)
                mark1.append(idx2)
                crossings.append(mark1)

    minimum = 9999999
    for crossing in crossings[1:]:
        sum = crossing[-2] + crossing[-1]
        if sum < minimum:
            minimum = sum

    if expected_val != 0:
        if expected_val == minimum:
            print("Test passed succesfully!")
    else:
        print(minimum)

# find_nearest_crossing(test_11, test_12, expected_test_val_1)
# find_nearest_crossing(test_21, test_22, expected_test_val_2)
find_nearest_crossing(path_1, path_2)
