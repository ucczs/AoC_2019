...
int main()
{
    uint64_t ore_neccessary = 0;
    uint64_t fuel_amount = 1'970'000;

    while (ore_neccessary <= max_ore)
    {

        g_all_chemicals.clear();

        CReactionList_Reader reaction_reader("input.txt");
        reaction_reader.read_reaction_list();

        CChemical* fuel = get_chemical("FUEL");
        fuel->neccessary_amount = fuel_amount;

        reaction_reader.count_overall_amount();

        CChemical* ore = get_chemical("ORE");
        ore_neccessary = ore->neccessary_amount;

        fuel_amount++;

        if (fuel_amount % 1'000 == 0)
        {
            cout << "Fuel: " << fuel_amount << ", ORE: " << ore_neccessary << endl;
        }
        
    }

    cout << "\nFuel amount with 1 trillion ore: " << fuel_amount-2 << endl;
