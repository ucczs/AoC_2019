...
enum Opcode_t {op_UNDEFINED = 0, op_ADD = 1, op_MULTIPLY = 2, op_SAVE_VAL = 3, op_OUTPUT_VAL = 4, op_HALT = 99};
enum ParaMode_t {POSITION_MODE = 0, IMMEDIATE_MODE = 1};
...
ParaMode_t get_parameter_mode_first(uint16_t instruction){
...
ParaMode_t get_parameter_mode_second(uint16_t instruction){
...
ParaMode_t get_parameter_mode_third(uint16_t instruction){
...
int64_t get_value_based_on_mode(ParaMode_t paramode, uint32_t idx, uint8_t idx_offset){
    if (paramode == IMMEDIATE_MODE)
    {
        return(g_diag_program_original[idx+idx_offset]);
    } else if (paramode == POSITION_MODE)
    {
        return(g_diag_program_original[g_diag_program_original[idx+idx_offset]]);
    } else
    {
        return(-1);
    }
...
int64_t get_parameter_value(uint32_t instruction_idx, uint8_t parameter_number){
    ...
    ParaMode_t paramode_first   = get_parameter_mode_first(instruction);
    ParaMode_t paramode_second  = get_parameter_mode_second(instruction);
    ...
    if( current_operation == op_ADD || current_operation == op_MULTIPLY )
    {
        ...
        case 2:
            ...
            break;
        case 3:
            return(get_value_based_on_mode(POSITION_MODE, instruction_idx, 2)); // for add and multiply always a write operation, which is always position mode
            break;                        
        default:
            return(-1);
            break;
...
uint64_t run_programm(uint8_t input_programm){
    ...
    while (current_op_code != op_HALT)
    {
        ...
        {
            diag_program[get_parameter_value(idx, 3)] = diag_program[get_parameter_value(idx, 1)] + diag_program[get_parameter_value(idx, 2)];
        } else if (current_op_code == op_MULTIPLY)
        {
            diag_program[get_parameter_value(idx, 3)] = get_parameter_value(idx, 1) * get_parameter_value(idx, 2);
        }
