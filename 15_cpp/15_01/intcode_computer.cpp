...
int64_t CIntcode_computer::run_programm(){
    uint32_t idx                    = 0;
    ...
    while (current_op_code != op_HALT)
    {
        ...
        {
            input_program = (int32_t)g_repair_droid.generate_newest_output();
        ...
        else if (current_op_code == op_OUTPUT_VAL)
        {
            ...
            //cout << "Output: " << output << "\n";   
            g_repair_droid.process_newest_input((position_type_t) output);
    ...
    g_repair_droid.draw_map();
    g_repair_droid.flood_area_with_oxygen();
    g_repair_droid.draw_map();
